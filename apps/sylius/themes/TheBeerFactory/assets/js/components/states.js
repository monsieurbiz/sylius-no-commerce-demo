...
function setClass(elem, stateClass, behaviour) {
  if (stateClass !== 'false') {
    if (behaviour === add) {
...
function setAria(elem, behaviour) {
...
function setTabindex(elem, tabindex, behaviour) {
  if (tabindex === 'true') {
    if (behaviour === add) {
...
function setState(parameters) {
  parameters.behaviours.forEach((behaviour, index) => {
    ...
    const tabindex =
      parameters.tabindexes !== null ? parameters.tabindexes[index] : null;
...
export default function state(elem) {
  const parameters = {
    ...
    tabindexes: elem.dataset.tabindex
      ? elem.dataset.tabindex.split(', ')
      : null,
